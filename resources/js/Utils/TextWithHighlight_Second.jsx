const TextWithHighlight_Second = ({ text = "", split = false, split_coma = false, split_dos_puntos = false }) => {
    const safeText = text || "";
    ...
    const renderHighlightedText = (textToRender) => {
        ...
        return parts.map((part, index) => {
            if (part.startsWith("*") && part.endsWith("*")) {
                return (
                    <span key={index} className="customtext-neutral-light">
                        {part.slice(1, -1)}
                    </span>
                );
...
export default TextWithHighlight_Second;